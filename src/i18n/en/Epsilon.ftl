...
# main
run-as-root = Running Epsilon as root is disallowed as it can lead to system breakage. Instead, Epsilon will prompt you when it needs superuser permissions.
...
clear-pkgbuild-cache = Clear Epsilon's internal PKGBUILD cache?
...
pacdiff-warning =
    ...
    You can suppress this warning in the future by setting `pacdiff_warn` to "false" in ~/.config/epsi/config.toml
