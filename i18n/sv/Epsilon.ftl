...
continue = Fortsätt?
# main
run-as-root = Att köra Epsilon som root är inte tillåtet eftersom det kan leda till att systemet går sönder. Istället, så kommer Epsilon att fråga dig när det behöver superanvändarbehörigheter.
...
clear-pkgbuild-cache = Rensa Epsilon interna PKGBUILD-cache?
...
pacdiff-warning =
    ...
    Du kan få tyst på denna varning i framtiden genom att ställa in `pacdiff_warn` till "false" i ~/.config/epsi/config.toml
# internal::paging
quit = avsluta
search = sök
